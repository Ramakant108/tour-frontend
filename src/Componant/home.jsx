...
const Home = ({ user }) => {
  ...
  const getdetail = async () => {
    setLoading(true);
    const res = await axios.get(`https://tour-backend-1wsb.onrender.com/tours/${user.id}/${title}`);
    setTours(res.data);
    setLoading(false)
    console.log(tours)
  };
  ...
  useEffect(() => {
    ...
    const fetchdata = async () => {
      setLoading(true);
      const data = await axios.get(`https://tour-backend-1wsb.onrender.com/tours/${user.id}`);
      setTours(data.data);
      setLoading(false)
    };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this tour?')) {
      await axios.delete(`https://tour-backend-1wsb.onrender.com/tours/${id}`);
      const data = await axios.get(`https://tour-backend-1wsb.onrender.com/tours/${user.id}`);
      setTours(data.data);
    }
  };
